<script lang="ts">
  ...
  import { statusColor, methodColor, pathOnly, domainOnly, formatSize, formatTime, formatDuration } from "$lib/utils";
  ...
  const keymap = new Keymap()
    .bind('l', () => {
      const el = document.getElementById('url-filter-input') as HTMLInputElement | null;
      el?.focus(); el?.select();
    ...
    <div class="flex items-center gap-4 h-full font-sans">
      ...
      <nav class="flex h-full ml-4">
        <button
          ...
          class="px-3 h-full text-[11px] font-bold transition-all border-b-2 {activeTab === 'requests' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700'}"
        >Requests</button>
        {#if !windowState.isScriptsPoppedOut}
          <button
            onclick={() => (activeTab = "scripts")}
            class="px-3 h-full text-[11px] font-bold transition-all border-b-2 {activeTab === 'scripts' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700'}"
          >Scripts</button>
        {/if}
      </nav>
    </div>
  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    {#if activeTab === "requests"}
      <PaneGroup direction="vertical" autoSaveId="requests-layout-v1">
        <!-- Top: Request List -->
        <Pane defaultSize={60} minSize={20} class="flex flex-col border-b border-slate-200 dark:border-[#30363d]">
          <div class="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#0d1117] border-r border-slate-200 dark:border-[#30363d]">
            {#if proxy.errorMsg}
              <div class="px-3 py-2 bg-red-500/10 border-b border-red-500/20 flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
                <div class="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></div>
                <div class="flex-1 min-w-0">
                  <p class="text-[10px] font-bold text-red-500 uppercase tracking-tighter">Proxy Error</p>
                  <p class="text-[10px] text-red-400 font-mono truncate">{proxy.errorMsg}</p>
                </div>
                <button
              ...
              <div class="flex gap-0.5 bg-slate-200/50 dark:bg-white/5 p-0.5 rounded-md border border-slate-200 dark:border-white/10">
                {#each ALL_METHODS as method}
                  <button
                    ...
                    class="px-1.5 py-0 text-[9px] font-mono font-bold rounded transition-colors {activeMethods.has(method) ? 'bg-white dark:bg-[#0d1117] text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}">{method}</button>
                {/each}
              </div>
              <div class="flex-1"></div>
              <button
                onclick={() => { proxy.clearTraffic(); displayLimit = 200; }}
            ...
            <div class="flex-1 overflow-y-auto no-scrollbar" bind:this={requestList} onscroll={onListScroll}>
              {#if filteredIds.length > displayLimit}
                <div class="py-4 text-center text-[10px] text-slate-400 font-mono border-b border-dashed border-slate-200 dark:border-[#30363d]">
                  {filteredIds.length - displayLimit} more historical requests available (scroll up to load)
                </div>
              {/if}
              {#each renderedIds as id (id)}
                ...
                {#if req}
                  <div
                    class="flex items-center px-2 py-1.5 text-xs cursor-pointer border-b border-black/5 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-[#1f2a3a] transition-colors {selectedId === id ? 'bg-indigo-50 dark:bg-[#1f2a3a] outline outline-1 -outline-offset-1 outline-indigo-400' : ''}"
                  ...
                  >
                    ...
                    <div class="w-16 shrink-0 font-mono font-black flex items-center gap-1.5 {methodColor(req.method)}">
                      {req.method}
                      {#if req.script_id !== "0"}
                        <span class="px-1 py-0 rounded-[3px] bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 text-[9px] font-black leading-none border border-amber-200 dark:border-amber-500/30 shadow-sm" title="Scripted">JS</span>
                      {/if}
                    </div>
                    ...
                    <div class="w-16 shrink-0 text-right text-slate-400">
                      {formatDuration(proxy.reqTime.get(id), proxy.resTime.get(id))}
                    </div>
        ...
        <PaneResizer class="h-1 bg-transparent hover:bg-indigo-500/20 cursor-row-resize transition-colors" />

        <!-- Bottom: Inspector -->
        <Pane defaultSize={40} minSize={10} class="flex flex-col min-h-0 bg-white dark:bg-[#0d1117]">
          {#if selectedReq}
            {#if windowState.poppedOutInspectors.has(selectedReq.id)}
              <div class="flex-1 flex flex-col items-center justify-center text-slate-400 bg-indigo-50/5 dark:bg-transparent">
                <p class="text-xs font-bold uppercase tracking-widest text-indigo-500/60">Inspector Detached</p>
                <p class="text-[10px] opacity-40 mt-1">This request is being inspected in a separate window</p>
                <button
                  onclick={() => windowState.toggleInspector(selectedReq!.id, false)}
            ...
            {/if}
          {:else}
            <div class="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/10 dark:bg-transparent">
              <p class="text-xs font-medium uppercase tracking-widest opacity-60">No Request Selected</p>
              <p class="text-[10px] opacity-40 mt-1 italic">Select traffic from the list above</p>
            </div>
          {/if}
        </Pane>
      </PaneGroup>
      ...
      <div class="flex-1 overflow-hidden flex flex-col">
        {#if windowState.isScriptsPoppedOut}
          <div class="flex-1 flex flex-col items-center justify-center text-slate-400">
            <p class="text-xs font-bold uppercase tracking-widest text-amber-500/60">Script Editor Detached</p>
            <p class="text-[10px] opacity-40 mt-1">Editing is currently active in a separate window</p>
            <button
              onclick={() => { windowState.toggleScripts(false); activeTab = "scripts"; }}
